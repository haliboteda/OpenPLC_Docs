...
- FQBN 和 sketch 路径能从缓存目录的 `build.options.json` 读出来，**别硬猜** —— 菜单项增删过几次，手写的 FQBN 会缺段
...
- **没有版本管理。** 工具不记录也不比较固件版本，签名验得过就烧 —— app 的版本是写它的人自己的事
...
**全部用例、它们覆盖哪条需求、最近一次跑出什么结果，在 `$PROD/docs/tables/STATUS.md`。**

> ⚠️ **这里以前有一张自己的总览表，2026-08-22 删掉了。** 它只列了 T / T1-11 / N 三组，而那时实际已经有 T2-01–T2-05、T1-12、T1-13、T1-21/T1-22、T1-14、T3-02、T3-03、T3-01、T1-17、DG1 和全部主机用例 —— **一份重复的表最后总是变成过期的那一份**，而它看起来和权威的那份一模一样。这正是这套文档反复踩的同一个坑。

## 排查间歇性故障的方法

2026-08-15 那个"UDP 发现偶发无应答"的经验，四条可复用：

**1. 短测抓不到周期性故障。** T1-01/T1-02/T1-03（几秒到一分钟）反复跑全是干净的；同一个问题在 25 分钟浸泡里稳定复现 18 次。**抽查式测试对"每 30 秒一次"这类故障基本无效** —— T1-04 就是为此存在的。
