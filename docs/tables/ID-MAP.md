...
|---|---|---|---|---|
| `A1`–`A7` `B1`–`B11` `C1`–`C14` `D1`–`D9` `E1`–`E8` `F1`–`F5` | **产品需求**（53 条，每条一句可判定的话） | [STATUS.md](STATUS.md) 第三节 | — 它们是主张，不是可执行的东西 | 同一张表的「状态」列 |
| `T1` `T1b` `T2` `T3` `T4` | **TCP 会话用例** | `$PROD/docs/tables/TEST-CASES.md` | `$TOOL:TestCase/tcp_session.go`（`register(testCase{id:"T1"…})`—— **编号写死在 Go 代码里，不能改**） | [STATUS.md](STATUS.md) 的「最近结果」列 |
| `N1`–`N5` | **UDP 发现用例** | `$PROD/docs/tables/TEST-CASES.md` | `$TOOL:TestCase/udp_discovery.go` | [STATUS.md](STATUS.md) 的「最近结果」列 |
| `S1` `S2` `S3` `S4a` `S4b` `G1` | **签名 / 掉电 / staging 用例** | `$PROD/docs/tables/TEST-CASES.md` | S1/S2 → `signature.go` + `signature_wrongkey.go`；S3 → `python tools/run_s3.py`；**S4a/S4b → `tools/run_s4.py`**；G1 → `python tools/run_case.py --case S1 --then-reset` | [STATUS.md](STATUS.md) 的「最近结果」列 |
